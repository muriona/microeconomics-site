...
Um consumidor tem a função de utilidade $U(x_1,x_2)=10x_1+5x_2+x_1x_2$ para o consumo entre Café (bem 1) e Croissants (bem 2). O preço do bem 1 é R\$ 2 e o preço do bem 2 é 4. Já, a renda do consumidor é R\$ 102 para um consumo mensal.

a)  Calcule a escolha ótima do consumidor

b)  Suponha que o preço do bem 1 passe a ser 3. O que a mudança sobre a escolha ótima do consumidor indica sobre a curva de preço-consumo nessa região?

## Questão 2

Resolva os seguintes problemas de maximização sujeita a uma restrição (ou seja, monte o Lagrangeano, ache as condições de primeira ordem e encontre os valores ótimos das variáveis de escolha):
...
Um consumidor tem uma renda de \$100,00 para escolher entre o consumo de dois bens. O preço do bem 1 é R\$ 4,00 e do bem 2 é R\$ 6,00.
...
Considere uma firma com função de produção $f(x_1,x_2)=x_1^{1/3}x_2^{2/3}$ e seja $w_1$ o preço do fator 1, $w_2$ o preço do fator 2 e $y$ o nível de produção.
...
Sejam 100 firmas idênticas atuando em concorrência perfeita. O custo total de curto prazo de uma delas é dado por $C(q)=0,5q^2+10q+5$.
...
Considere a seguinte curva de custo de uma firma competitiva: $c(y)=4y^2+400$.
...
Suponha que um produtor produça azeite com a seguinte função de custo de curto prazo: \$C(y)=y\^{a}+F\$, onde $a>1$ e $F$ uma constante.
