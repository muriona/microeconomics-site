...
A curva de indiferença tem a forma: $U(x,y)=kx^ay^b$


```{r}

sliderInput("k",
            label = "Selecione o coeficiente 'k'",
            min = 0.1,
            max = 3,
            value = 1,
            step = 0.05,
...
utility_y <- function(my_x, my_U) {
  my_U^(1/input$coefy)/(k^(1/input$coefy)*my_x^(coefx/coefy))
}
...
indif_curva <- data_ci %>% 
  ...
  annotate(geom="text",
           x = 80,
           y= 51,
           label = glue("Função de Utilidade na forma: U = {input$k}x^{input$coefx} * y^{input$coefy}"),
           color = "darkblue")+
  labs(x="Unidades do bem X", y="Unidades do bem Y")+
  scale_x_continuous(expand = c(0, 0), breaks = seq(0, 100, 10)) +
  scale_y_continuous(expand = c(0, 0), breaks = seq(0, 100, 10)) +
...
utility_y <- function(my_x, my_U) {
  my_U^(1/input$pcoefy)/(pk^(1/input$pcoefy)*my_x^(pcoefx/pcoefy))
}
